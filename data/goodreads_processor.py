"""Converts Goodreads review JSON records into user profiles."""
        ...
        for row in records:
            grouped[row["user_id"]].append(
                ReviewRecord(
                    ...
                    metadata={"title": row.get("title", "Unknown title")},
                )
            )

        profiles: list[UserProfile] = []
        for user_id, reviews in grouped.items():
            profile = self.profile_builder.build(user_id=user_id, source="goodreads", reviews=reviews)
            if profile:
