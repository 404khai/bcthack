"""Converts Yelp review JSON records into user profiles."""
        ...
        for row in records:
            grouped[row["user_id"]].append(
                ReviewRecord(
                    ...
                    metadata={"business_name": row.get("business_name", "Unknown business")},
                )
            )

        profiles: list[UserProfile] = []
        for user_id, reviews in grouped.items():
            profile = self.profile_builder.build(user_id=user_id, source="yelp", reviews=reviews)
            if profile:
